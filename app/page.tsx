...
// ------------------------------------------------------------------
// ▼ データ定義：メモ帳(page.tsx日本語.txt)から100%完全移植
// ------------------------------------------------------------------
const useCases = [
  {
    ...
    visual: (
      <div className="h-full bg-slate-50 flex flex-col rounded-lg border border-zinc-200 p-4 relative overflow-hidden">
        ...
        <div className="flex-grow flex items-center justify-around relative z-10 px-2 pb-4">
          <div className="flex flex-col items-center w-1/3">
            <div className="text-xs md:text-base font-bold text-red-500 mb-2 bg-red-50 px-2 py-0.5 rounded-full">Before (60分)</div>
            <div className="relative w-24 h-32 md:w-48 md:h-64 flex items-center justify-center">
          ...
          <div className="flex flex-col items-center w-1/3 text-center">
            <div className="text-xs md:text-base font-bold text-emerald-600 mb-2 bg-emerald-50 px-2 py-0.5 rounded-full">After (5分)</div>
            <div className="w-20 h-28 md:w-40 md:h-56 bg-gradient-to-br from-emerald-400 to-blue-500 rounded-xl shadow-lg flex flex-col items-center justify-center text-white z-30">
  ...
  },
  {
    ...
    benefit: (
      <>
        人による間違いを<span className="font-bold border-b border-emerald-500/50">37%から5%</span>(※1)に減らし、売上を最大<span className="font-bold border-b border-emerald-500/50">5.5%</span>(※2)向上させます。
      </>
    ),
    visual: (
  ...
  { id: 3, icon: "📞", title: "３．高度な顧客対応の自動化", target: "営業・CS", desc: "NLP技術で電話やメールの一次対応を自動化。", detail: "顧客からの電話音声をテキスト化し、社内FAQから最適な回答をAIが生成します。", benefit: "24時間365日の対応が可能になり、夜間休日の機会損失を防ぎます。", visual: <div className="h-full bg-slate-100 p-4 flex items-center justify-center text-slate-400 italic font-bold">Chat UI Mockup</div> },
  { id: 4, icon: "🚨", title: "４．設備故障の予知・検知", target: "製造・ビル管理", desc: "異音や振動データから故障の予兆を察知。", detail: "熟練工が判断していた異常をAIが数値化。ライン停止前にアラートを出します。", benefit: "予期せぬダウンタイムを回避し、計画的なメンテナンスを実現します。", visual: <div className="h-full bg-zinc-900 p-4 flex items-center justify-center text-emerald-500 font-mono">Sensory Analysis System</div> },
  { id: 5, icon: "🧾", title: "５．経理・仕訳業務の自動化", target: "経理・総務", desc: "領収書読み取りと勘定科目の自動仕訳。", detail: "OCRとAIを組み合わせ、バラバラな請求書を読み取り、会計ソフトへ入力します。", benefit: "手入力の手間をなくし、月末の経理負担を劇的に軽減します。", visual: <div className="h-full bg-slate-50 p-4 flex items-center justify-center text-blue-500 font-bold">Accounting Automation View</div> },
  { id: 6, icon: "🔎", title: "６．社内ナレッジのAI検索", target: "全業種", desc: "社内のバラバラな資料を統合し、対話形式で検索。", detail: "社内版ChatGPT環境を構築。質問するだけでマニュアルから回答が得られます。", benefit: "「あの資料どこだっけ？」という時間をゼロにします。", visual: <div className="h-full bg-white p-4 flex items-center justify-center text-slate-300 font-bold">Knowledge Search Interface</div> }
];
            ...
            <section className="mb-24 bg-blue-50/50 -mx-6 px-6 py-16">
              <div className="max-w-5xl mx-auto text-center">
                ...
                <div className="grid gap-16 md:grid-cols-4 md:gap-8 relative text-left">
                  {[
                    { s: "01", t: "ヒアリング", d: "現状の課題や理想をお聞かせください。" },
                    { s: "02", t: "ご提案・PoC", d: "解決策をご提案。試作版で効果を検証します。" },
                    { s: "03", t: "本開発・実装", d: "業務で使えるシステムを開発・導入します。" },
                    { s: "04", t: "運用・サポート", d: "導入後の使い方のサポートも継続します。" },
                  ].map((item, i) => (
            ...
            <footer className="mt-auto border-t border-zinc-200 pt-10 pb-6 text-zinc-600">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10 text-left">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-zinc-500">Email:</p><a className="text-blue-600 text-sm break-all">tetsuya.nakajima@smartcityai.co.jp</a>
                </div>
              </div>
            ...
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setSelectedCase(null)}>
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl h-[90vh] overflow-hidden flex flex-col md:flex-row" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="w-full md:w-[30%] p-8 overflow-y-auto">
                  ...
                  <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 mb-6"><h4 className="text-xs font-bold text-emerald-700 mb-2 uppercase">✨ Benefit</h4><div className="text-emerald-900 text-sm leading-relaxed">{selectedCase.benefit}</div></div>
          ...
          {isCalendarOpen && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-0 md:p-4 bg-black/80 backdrop-blur-sm" onClick={() => setIsCalendarOpen(false)}>
              <div className="bg-white md:rounded-lg shadow-2xl w-full max-w-5xl h-full md:h-[90vh] overflow-hidden flex flex-col relative" onClick={(e) => e.stopPropagation()}>
                <div className="bg-emerald-600 text-white p-4 flex justify-between items-center shadow-md"><div className="font-bold text-lg flex items-center gap-2">📅 無料相談 予約カレンダー</div><button onClick={() => setIsCalendarOpen(false)} className="bg-
