...
// ------------------------------------------------------------------
// ▼ データ定義：メモ帳(page.tsx日本語.txt)の全156行を完全移植 [cite: 2]
// ------------------------------------------------------------------
const useCases = [
  {
    ...
    benefit: (
      <>
        ...
        <span className="block mt-3 pt-2 border-t border-emerald-200/50 text-[10px] text-emerald-800/70 font-normal leading-snug">
          ※1 Noy, S., & Zhang, W. (2023). Science. ※2 [当社試算] 従来60分の業務を5分に短縮した場合。 [cite: 3, 5]
        </span>
      </>
    ),
    visual: (
        ...
        <div className="flex-grow flex items-center justify-around relative z-10 px-2 pb-4">
          <div className="flex flex-col items-center group relative w-1/3 text-center">
            ...
            <div className="relative w-24 h-32 md:w-48 md:h-64 flex items-center justify-center mt-2">
               ...
               <div className="absolute w-20 h-28 md:w-40 md:h-56 bg-white border border-zinc-300 shadow-sm transform rotate-6 translate-x-[-5px] z-10"></div>
               <div className="absolute w-20 h-28 md:w-40 md:h-56 bg-white border border-zinc-300 shadow-md transform -rotate-3 z-20"></div>
          ...
          <div className="flex flex-col items-center justify-center z-40 -mt-4">
            ...
            <div className="font-black text-xl md:text-4xl text-white bg-gradient-to-r from-red-500 to-emerald-500 px-3 py-1 rounded-lg shadow-xl animate-bounce border-2 border-white">-90%</div>
          </div>
          <div className="flex flex-col items-center group relative w-1/3 text-center">
  ...
  },
  {
    ...
    detail: "倉庫内のカメラ映像やRFIDをAIが解析。製品の数をカウントしたり、異常を検知します。",
    benefit: (
      <>
      ...
      <div className="h-full bg-black rounded-lg overflow-hidden relative">
        ...
        <div className="absolute inset-0 p-4 font-mono text-xs text-emerald-400">
          ...
          <div className="absolute bottom-0 left-0 w-full bg-black/80 border-t border-zinc-700 p-3 flex justify-around text-center text-white">
            <div><div className="text-[10px] text-zinc-400 uppercase tracking-widest">Total Items</div><div className="text-xl font-bold">842</div></div>
            <div><div className="text-[10px] text-zinc-400 uppercase tracking-widest">Status</div><div className="text-xl font-bold text-emerald-400">OK</div></div>
          </div>
        </div>
      </div>
    )
  },
  { id: 3, icon: "📞", title: "３．高度な顧客対応の自動化", target: "営業・CS", desc: "NLP技術で電話やメールの一次対応を自動化。", detail: "顧客からの電話音声をテキスト化し、最適な回答をAIが生成します。", benefit: "24時間365日の対応が可能になります。", visual: <div className="h-full bg-slate-100 flex items-center justify-center text-slate-400 italic">Chat System Interface [cite: 27]</div> },
  { id: 4, icon: "🚨", title: "４．設備故障の予知・検知", target: "製造・ビル管理", desc: "異音や振動データから故障の予兆を察知。", detail: "熟練工の「音」の判断を数値化。ライン停止前にアラートを出します。", benefit: "予備保全により、計画的なメンテナンスを実現します。", visual: <div className="h-full bg-zinc-900 flex items-center justify-center text-emerald-500 font-mono">Sensory Analysis System [cite: 41]</div> },
  { id: 5, icon: "🧾", title: "５．経理・仕訳業務の自動化", target: "経理・総務", desc: "領収書読み取りと勘定科目の自動仕訳。", detail: "OCRとAIを組み合わせ、会計ソフトへの入力を自動化します。", benefit: "月末の経理負担を劇的に軽減します。", visual: <div className="h-full bg-slate-50 flex items-center justify-center text-blue-500 font-bold">Accounting Automation View [cite: 54]</div> },
  { id: 6, icon: "🔎", title: "６．社内ナレッジのAI検索", target: "全業種", desc: "社内のバラバラな資料を統合し、対話形式で検索。", detail: "社内版ChatGPTのような環境を構築。マニュアル検索をゼロにします。", benefit: "探し物の時間をゼロにします。", visual: <div className="h-full bg-white flex items-center justify-center text-slate-300 font-bold">Knowledge Search Interface [cite: 68]</div> }
];

const faqs = [
  { q: "AIの知識が全くなくても大丈夫ですか？", answer: "はい、全く問題ありません。専門知識は一切不要です。", detail: "技術的な選定、開発、運用はすべて弊社が担当します。" },
  { q: "費用はどのくらいかかりますか？", answer: "スモールスタートを推奨しており、数十万円からのプランもございます。", detail: "IT導入補助金などの公的支援の申請支援も行います。" }
];
          ...
          <div className="mx-auto flex min-h-screen max-w-5xl flex-col px-6 py-16">
            ...
            <header className="mb-20 border-b border-zinc-200 pb-12">
              <div className="flex items-center space-x-3 mb-8">
                <Image src="/logo.png" alt="Smart City AI LLC" width={160} height={50} className="h-auto w-24 sm:w-32" priority />
                <p className="text-sm font-bold tracking-[0.1em] text-zinc-900 uppercase">Smart City AI LLC [cite: 90]</p>
              </div>
              ...
              <p className="mt-6 max-w-2xl text-lg leading-relaxed text-zinc-700">
                「AIを使いたいが、何から始めればいいかわからない」経営者様の悩みを解決します。 [cite: 92]
              </p>
              <div className="mt-8">
                <button onClick={() => setIsCalendarOpen(true)} className="rounded-lg bg-emerald-600 px-8 py-4 text-white font-bold shadow-xl transition hover:bg-emerald-700">まずは無料相談で可能性を探る → [cite: 93]</button>
              </div>
            </header>

            {/* 導入の流れ (矢印図解含む) [cite: 100-109] */}
            <section className="mb-24 bg-blue-50/50 -mx-6 px-6 py-16">
              <div className="max-w-5xl mx-auto text-center">
                ...
                <h3 className="text-2xl font-bold text-blue-900 mb-12">ご相談から導入までの流れ [cite: 100]</h3>
                <div className="grid gap-16 md:grid-cols-4 md:gap-8 relative text-left">
                  {[
                    { s: "01", t: "ヒアリング", d: "課題や理想をお聞かせください。" },
                    { s: "02", t: "ご提案・PoC", d: "解決策をご提案。効果を検証します。" },
                    { s: "03", t: "本開発・実装", d: "本システムを開発・導入します。" },
                    { s: "04", t: "運用・サポート", d: "使い方のサポートも継続します。" },
                  ].map((item, i) => (
            ...
            <section className="mb-24">
              <div className="text-center mb-12"><h2 className="text-sm font-bold text-emerald-600 uppercase tracking-widest">Solutions</h2><h3 className="text-2xl font-bold text-blue-900 mt-2">AI導入で解決できる 6つの業務課題 [cite: 110]</h3></div>
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 text-left">
                {useCases.map((item) => (
                  <button key={item.id} onClick={() => setSelectedCase(item)} className="group bg-white p-6 rounded-xl border border-zinc-200 hover:border-blue-400 hover:shadow-lg transition flex flex-col h-full [cite: 111]">
            ...
            <footer className="mt-auto border-t border-zinc-200 pt-10 pb-6 text-zinc-600 text-left">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
                <div><h3 className="text-sm font-bold text-zinc-900 mb-3 uppercase">Smart City AI LLC [cite: 126]</h3><p className="text-xs text-zinc-500 font-bold">代表社員</p><p className="font-medium text-zinc-900 mb-3">中島 徹也 </p></div>
                <div><h3 className="text-sm font-bold text-zinc-900 mb-3 uppercase">Contact [cite: 128]</h3><a className="text-blue-600 text-sm break-all" href="mailto:tetsuya.nakajima@smartcityai.co.jp">tetsuya.nakajima@smartcityai.co.jp [cite: 130]</a></div>
                <div className="text-center md:text-right text-xs pt-6">© 2025 Smart City AI LLC. [cite: 131]</div>
              </div>
            </footer>
          </div>

          {/* ========================================================= */}
          {/* 【重要修正：メモ帳の「✕」ボタンを完全復元】 [cite: 132-146] */}
          {/* ========================================================= */}
          {selectedCase && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-2 md:p-4 bg-black/80 backdrop-blur-sm" onClick={() => setSelectedCase(null)}>
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl h-[95vh] overflow-hidden relative flex flex-col md:flex-row text-left" onClick={(e) => e.stopPropagation()}>
                <div className="w-full md:w-[70%] bg-zinc-900 p-4 relative">
                   {/* ▼ スマホ用「✕」ボタン [cite: 134] */}
                   <button onClick={() => setSelectedCase(null)} className="md:hidden absolute top-4 right-4 z-50 bg-white text-zinc-900 shadow-xl rounded-full w-12 h-12 flex items-center justify-center font-bold text-2xl">✕</button>
                   <div className="h-full bg-black rounded-lg overflow-hidden flex items-center justify-center">{selectedCase.visual}</div>
                </div>
                <div className="w-full md:w-[30%] flex flex-col bg-white h-full relative">
                  {/* ▼ PC用「✕ 閉じる」ボタン [cite: 137] */}
                  ...
                  <div className="p-8 overflow-y-auto flex-grow">
                    ...
                    <p className="text-sm text-zinc-600 mb-6">{selectedCase.detail}</p>
                    ...
                    <button onClick={() => { setSelectedCase(null); setIsCalendarOpen(true); }} className="w-full bg-blue-600 text-white font-bold py-4 rounded-xl shadow-lg">無料相談を申し込む</button>
                  </div>
                </div>
              </div>
            </div>
          ...
          {isCalendarOpen && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-0 md:p-4 bg-black/80 backdrop-blur-sm" onClick={() => setIsCalendarOpen(false)}>
              <div className="bg-white md:rounded-lg shadow-2xl w-full max-w-5xl h-full md:h-[90vh] overflow-hidden flex flex-col relative text-left" onClick={(e) => e.stopPropagation()}>
                <div className="bg-emerald-600 text-white p-4 flex justify-between items-center"><div className="font-bold text-lg flex items-center gap-2">📅 無料相談 予約カレンダー</div><button onClick={() => setIsCalendarOpen(false)} className="bg-emerald-500 px-4 py-2 rounded-full font-bold">✕ 閉じる [cite: 150]</button></div>
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-[#ececec] font-sans py-24 px-6 text-left">
          <div className="max-w-3xl mx-auto">
            <header className="border-l-4 border-white pl-6 mb-16">
              ...
              <p className="text-xl text-[#888] font-medium mb-2">Operationalizing Urban Intelligence for National-Scale Infrastructure.</p>
              <p className="text-sm font-bold">MS in Computer Science (AI), Georgia Tech | World Smart City Award Grand Prix Winner</p>
            </header>
            <section className="mb-12"><h2 className="text-2xl font-bold border-b border-[#333] pb-2 mb-6 uppercase tracking-widest">The Mission</h2><p className="text-lg leading-relaxed text-[#bbb]">Translating complex urban environments into executable data intelligence. Based on <strong>Reinforcement Learning</strong> and public sector leadership, I operationalize AI where policy meets reality.</p></section>
            <div className="grid md:grid-cols-2 gap-8 mb-16">
              ...
              <div className="bg-[#111] border border-[#222] p-8 hover:border-[#444] transition"><h3 className="text-lg font-bold mb-4 uppercase">Crisis Response</h3><p className="text-sm text-[#888]">Architected disaster preparedness systems for <strong>proactive command structures</strong>.</p></div>
            </div>
            <div className="bg-[#1a1a1a] p-6 rounded text-sm border-l-2 border-emerald-500 font-bold">Status: Based in Yokohama | <strong>US Green Card Holder</strong></div>
            <footer className="mt-16 pt-8 border-t border-[#222] text-[#555] flex gap-6 font-bold"><a href="https://www.linkedin.com/in/tnakajima2050/" target="_blank" className="text-white hover:underline">LinkedIn Profile</a><span>|</span><span>Smart City AI LLC</span></footer>
          </div>
        </div>
      )}
    </div>
