...
// ------------------------------------------------------------------
// ▼ データ定義：メモ帳(page.tsx日本語.txt)の内容を100%完全移植
// ------------------------------------------------------------------
const useCases = [
  {
    id: 1,
    ...
    benefit: (
      <>
        ...
        <span className="block mt-3 pt-2 border-t border-emerald-200/50 text-[10px] text-emerald-800/70 font-normal leading-snug">
          ※1 Noy, S., & Zhang, W. (2023). Science.<br/>
        ...
        <div className="flex-grow flex items-center justify-around relative z-10 px-2 pb-4">
          <div className="flex flex-col items-center group relative w-1/3">
            ...
            <div className="relative w-24 h-32 md:w-48 md:h-64 flex items-center justify-center mt-2">
               <div className="absolute w-20 h-28 md:w-40 md:h-56 bg-white border border-zinc-300 shadow-sm transform -rotate-12 translate-x-2 translate-y-2 z-0 flex items-center justify-center"></div>
          ...
          <div className="flex flex-col items-center justify-center z-40 -mt-4">
            ...
            <div className="font-black text-xl md:text-4xl text-white bg-gradient-to-r from-red-500 to-emerald-500 px-3 py-1 md:px-5 md:py-2 rounded-lg shadow-xl transform -rotate-6 border-2 border-white mt-1 whitespace-nowrap animate-bounce">-90%</div>
          </div>
          <div className="flex flex-col items-center group relative w-1/3">
            ...
            <div className="relative w-24 h-32 md:w-48 md:h-64 flex items-center justify-center mt-2">
               <div className="w-20 h-28 md:w-40 md:h-56 bg-gradient-to-br from-emerald-400 to-blue-500 rounded-xl shadow-lg flex flex-col items-center justify-center text-white border-t border-white/30 p-2 z-30 relative overflow-hidden">
  ...
  },
  {
    ...
    benefit: (
      <>
        数え間違いを<span className="font-bold border-b border-emerald-500/50">37%から5%</span>(※1)に減らし、売上を最大<span className="font-bold border-b border-emerald-500/50">5.5%</span>(※2)向上させます。
      </>
    ),
    visual: (
  ...
  { id: 3, icon: "📞", title: "３．高度な顧客対応の自動化", target: "営業・CS", desc: "NLP技術で電話やメールの一次対応を自動化。", detail: "顧客からの電話音声をテキスト化し、最適な回答をAIが生成します。", benefit: "24時間365日の対応が可能になります。", visual: <div className="h-full bg-slate-100 flex items-center justify-center text-slate-400 italic">Chat UI Mockup</div> },
  ...
  { id: 5, icon: "🧾", title: "５．経理・仕訳業務の自動化", target: "経理・総務", desc: "領収書読み取りと勘定科目の自動仕訳。", detail: "OCRとAIを組み合わせ、会計ソフトへの入力を自動化します。", benefit: "月末の経理負担を劇的に軽減します。", visual: <div className="h-full bg-slate-50 flex items-center justify-center text-blue-500 font-bold">Accounting Automation</div> },
  { id: 6, icon: "🔎", title: "６．社内ナレッジのAI検索", target: "全業種", desc: "社内のバラバラな資料を統合し、対話形式で検索。", detail: "社内版ChatGPT環境を構築し、マニュアル検索を高速化します。", benefit: "探し物の時間をゼロにします。", visual: <div className="h-full bg-white flex items-center justify-center text-slate-300 font-bold">Knowledge Search Interface</div> }
];
  ...
  return (
    <div className="relative">
      ...
      {lang === 'jp' ? (
        ...
        <main className="min-h-screen bg-zinc-50 text-zinc-900 font-sans">
          <div className="mx-auto flex min-h-screen max-w-5xl flex-col px-6 py-16">
              ...
              <div className="max-w-5xl mx-auto">
                ...
                <div className="grid gap-16 md:grid-cols-4 md:gap-8 relative text-left">
                  {[
                    ...
                    { s: "03", t: "本開発・実装", d: "検証結果に基づき、本システムを開発・導入します。" },
                    ...
                    <div key={i} className="relative bg-white p-5 rounded-lg border border-blue-100 shadow-sm z-10 h-full">
                      ...
                      <p className="text-xs text-zinc-600 mt-2 leading-5">{item.d}</p>
                      {i < 3 && (
              ...
              <div className="space-y-4">
                {faqs.map((faq, index) => (
                  <div key={index} className="border border-zinc-200 rounded-lg bg-white overflow-hidden shadow-sm">
                    <button onClick={() => toggleFaq(index)} className="w-full flex justify-between p-5 text-left bg-zinc-50 hover:bg-zinc-100">
            ...
            <footer className="mt-auto border-t border-zinc-200 pt-10 pb-6 text-zinc-600">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10 text-left">
                <div><h3 className="text-sm font-bold text-zinc-900 mb-3 uppercase">Smart City AI LLC</h3><p className="text-xs">横浜を拠点に、日本全国の企業のAI導入・DX推進をサポートしています。</p></div>
                <div><h3 className="text-sm font-bold text-zinc-900 mb-3 uppercase">Menu</h3><ul className="text-sm space-y-2"><li>ホーム</li><li>無料相談・お問い合わせ</li></ul></div>
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 mb-3 uppercase">Contact</h3>
          ...
          {selectedCase && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setSelectedCase(null)}>
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl h-[90vh] overflow-hidden flex flex-col md:flex-row text-left" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="w-full md:w-[30%] p-8 overflow-y-auto">
                  ...
                  <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 mb-6">
                    <h4 className="text-xs font-bold text-emerald-700 mb-2 uppercase tracking-widest">✨ Benefit</h4>
                    <div className="text-emerald-900 text-sm leading-relaxed">{selectedCase.benefit}</div>
                  </div>
          ...
          {isCalendarOpen && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-0 md:p-4 bg-black/80 backdrop-blur-sm" onClick={() => setIsCalendarOpen(false)}>
              <div className="bg-white md:rounded-lg shadow-2xl w-full max-w-5xl h-full md:h-[90vh] overflow-hidden flex flex-col relative text-left" onClick={(e) => e.stopPropagation()}>
                <div className="bg-emerald-600 text-white p-4 flex justify-between items-center"><div className="font-bold text-lg flex items-center gap-2">📅 無料相談 予約カレンダー</div><button onClick={() => setIsCalendarOpen(false)} className="bg-emerald-500 px-4 py-2 rounded-full font-bold">✕ 閉じる</button></div>
        ...
        /* 【英語サイト：パランティア特化型】 */
        /* ========================================================= */
        <div className="min-h-screen bg-[#0a0a0a] text-[#ececec] font-sans py-24 px-6">
          <div className="max-w-3xl mx-auto text-left">
            <header className="border-l-4 border-white pl-6 mb-16">
              <div className="inline-block bg-white text-black text-[10px] font-black px-2 py-1 mb-4 uppercase">FORWARD DEPLOYED STRATEGIST</div>
              <h1 className="text-5xl font-black tracking-tighter mb-2">Tatsuya Nakajima</h1>
              <p className="text-xl text-[#888] mb-2 font-medium">Operationalizing Urban Intelligence for National-Scale Infrastructure.</p>
              <p className="text-sm font-bold">MS in Computer Science (AI), Georgia Tech | World Smart City Award Grand Prix Winner</p>
            </header>

            <section className="mb-12">
              <h2 className="text-2xl font-bold border-b border-[#333] pb-2 mb-6 uppercase tracking-widest">The Mission</h2>
              <p className="text-lg leading-relaxed text-[#bbb]">
                I specialize in translating complex urban environments into executable data intelligence. Based on <strong>Reinforcement Learning</strong> and public sector leadership, I operationalize AI where policy meets reality.
              </p>
            </section>

            <div className="grid md:grid-cols-2 gap-8 mb-16">
              <div className="bg-[#111] border border-[#222] p-8 hover:border-[#444] transition">
                <h3 className="text-lg font-bold mb-4 uppercase">Operational Excellence</h3>
                <p className="text-sm text-[#888]">Led the <strong>Yokohama Smart City Project (YSCP)</strong>. Managed grid energy policy for 3.7 million citizens with complex stakeholder alignment.</p>
              </div>
              <div className="bg-[#111] border border-[#222] p-8 hover:border-[#444] transition">
                <h3 className="text-lg font-bold mb-4 uppercase">Crisis Response</h3>
                <p className="text-sm text-[#888]">Architected data-driven disaster preparedness systems, transforming reactive protocols into <strong>proactive command structures</strong>.</p>
              </div>
            </div>

            <div className="bg-[#1a1a1a] p-6 rounded-sm text-sm border-l-2 border-emerald-500">
              <span className="font-bold text-white uppercase mr-4">Status:</span> Based in Yokohama/Remote | <strong>US Green Card Holder</strong>
            </div>

            <footer className="mt-16 pt-8 border-t border-[#222] text-[#555]">
              <div className="flex gap-6 items-center">
                <a href="https://www.linkedin.com/in/tnakajima2050/" target="_blank" className="text-white hover:underline font-bold">LinkedIn Profile</a>
                <span className="text-[#333]">|</span>
                <span>Smart City AI LLC</span>
              </div>
            </footer>
          </div>
        </div>
