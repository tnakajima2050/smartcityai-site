...
const useCases = [
  {
    ...
    benefit: (
      <>
        生成AIにより文書作成時間を<span className="font-bold border-b border-emerald-500/50">40%短縮</span>(※1)し、最大<span className="font-bold border-b border-emerald-500/50">90%の工数削減</span>(※2)を実現します。
        ...
        <div className="flex-grow flex items-center justify-around relative z-10 px-2 pb-4">
          <div className="flex flex-col items-center w-1/3">
            ...
            <div className="relative w-24 h-32 md:w-48 md:h-64 flex items-center justify-center">
               ...
               <div className="absolute w-20 h-28 md:w-40 md:h-56 bg-red-50 border-2 border-red-400 shadow-lg transform rotate-2 z-30 flex flex-col items-center justify-center p-1">
                  ...
                  <div className="text-[9px] md:text-sm font-bold text-red-600 text-center">手入力<br/>転記地獄</div>
               </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center w-1/3">
            ...
            <div className="w-20 h-28 md:w-40 md:h-56 bg-gradient-to-br from-emerald-400 to-blue-500 rounded-xl shadow-lg flex flex-col items-center justify-center text-white z-30">
               ...
               <div className="text-[10px] md:text-sm font-bold text-center">AI<br/>自動生成</div>
            </div>
          </div>
        </div>
  ...
  { id: 3, icon: "📞", title: "３．高度な顧客対応の自動化", target: "営業・CS", desc: "NLP技術で電話やメールの一次対応を自動化。", detail: "顧客からの電話音声をテキスト化し、社内FAQから最適な回答をAIが生成します。", benefit: "24時間365日の対応が可能になり、夜間休日の機会損失を防ぎます。", visual: <div className="h-full bg-slate-100 p-4 flex items-center justify-center text-slate-400 italic">Chat System View (Simplified)</div> },
  { id: 4, icon: "🚨", title: "４．設備故障の予知・検知", target: "製造・ビル管理", desc: "異音や振動データから故障の予兆を察知。", detail: "熟練工が「音」で判断していた異常をAIが数値化。ライン停止前にアラートを出します。", benefit: "予備保全により、計画的なメンテナンスを実現します。", visual: <div className="h-full bg-zinc-900 p-4 flex items-center justify-center text-emerald-500 font-mono">Sensory Analysis System</div> },
  ...
  { id: 6, icon: "🔎", title: "６．社内ナレッジのAI検索", target: "全業種", desc: "社内のバラバラな資料を統合し、対話形式で検索。", detail: "社内版ChatGPT環境を構築。質問するだけでマニュアルから回答が得られます。", benefit: "探し物の時間をゼロにします。", visual: <div className="h-full bg-white p-4 flex items-center justify-center text-slate-300">Knowledge Search Interface</div> }
];
          ...
          <div className="mx-auto flex min-h-screen max-w-5xl flex-col px-6 py-16">
            ...
            <header className="mb-20 border-b border-zinc-200 pb-12">
              ...
              <p className="mt-6 max-w-2xl text-lg leading-relaxed text-zinc-700">
                「AIを使いたいが、何から始めればいいかわからない」経営者様の悩みを、確かな技術とスモールスタート戦略で解決します。
              </p>
              <div className="mt-8 flex flex-col sm:flex-row gap-4">
                <button onClick={() => setIsCalendarOpen(true)} className="rounded-lg bg-emerald-600 px-8 py-4 text-white font-bold shadow-xl transition hover:bg-emerald-700 hover:-translate-y-0.5">
                  まずは無料相談で可能性を探る →
                </button>
              </div>
            </header>
            ...
            {/* 5. FAQ */}
            <section className="mb-24 max-w-3xl mx-auto w-full text-center">
              <h2 className="text-2xl font-bold text-blue-900 mb-10">よくあるご質問</h2>
            ...
            <section className="mb-20 bg-zinc-900 text-zinc-300 rounded-2xl p-8 md:p-12 flex flex-col md:flex-row items-center gap-8">
              <div className="flex-1">
                ...
                <p className="text-sm leading-relaxed mb-6">AIは人手不足に悩む企業にこそ、その恩恵が必要です。デジタルパートナーとして伴走します。</p>
              </div>
            ...
            <footer className="border-t border-zinc-200 pt-10 pb-6 text-zinc-600">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
                <div><h3 className="text-sm font-bold text-zinc-900 mb-3 uppercase">Smart City AI LLC</h3><p className="text-xs">横浜を拠点に、AI導入・DX推進をサポートしています。</p></div>
            ...
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setSelectedCase(null)}>
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl h-[90vh] overflow-hidden flex flex-col md:flex-row" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="w-full md:w-[30%] p-8 overflow-y-auto">
                  ...
                  <button onClick={() => { setSelectedCase(null); setIsCalendarOpen(true); }} className="w-full bg-blue-600 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-blue-700">無料相談を申し込む</button>
                </div>
              </div>
            </div>
          )}
          {isCalendarOpen && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-
